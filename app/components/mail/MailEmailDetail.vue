...
<template>
  <div class="flex-1 h-full min-h-0 flex flex-col">
    <div class="rounded-xl border bg-card shadow-sm overflow-hidden flex flex-col h-full">
      <div v-if="!email" class="flex flex-col items-center justify-center h-full py-12 text-muted-foreground/30">
      ...
      <div v-else class="flex flex-col h-full min-h-0 overflow-hidden">
        <div class="p-3 md:p-4 border-b border-border shrink-0 bg-card z-10">
          <div class="flex items-center gap-2 mb-1.5">
            <span
              class="inline-flex items-center rounded-md border border-border px-2 py-0.5 font-mono text-[10px] tracking-widest uppercase font-bold"
              :class="email.seen ? 'bg-muted text-muted-foreground' : 'bg-primary text-primary-foreground'">
              {{ email.seen ? 'ĐÃ ĐC' : 'MỚI' }}
            </span>
            <span class="font-mono text-[10px] tracking-widest uppercase text-muted-foreground">{{
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs">
            ...
            <span class="font-mono text-[11px] text-muted-foreground break-all">→ {{ address }}</span>
          </div>
        </div>
        <div
